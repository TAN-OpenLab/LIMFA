...
class MultiHeadAttn_Root_Enhence(nn.Module):
    def __init__(self, h_in, n_head, dropout=0.1, attn_scale=False, root_enhance= True):
        """
        ...
        if root_enhance:
            self.q_linear = nn.Linear(h_in,  h_in)
            self.kv_linear = nn.Linear(2 * h_in, 2 * h_in)
        else:
            self.qkv_linear = nn.Linear(h_in, 3* h_in)
    ...
    def forward(self, x, mask_nonzero):
        ...
        if self.root_enhance:
            ...
            q = self.q_linear(x)
            mask_nonzero_matrix = torch.clone(q)
            (batch, row) = mask_nonzero
            mask_nonzero_matrix[batch, row, :] = torch.zeros_like(mask_nonzero_matrix[0, 0, :])
            q = q - mask_nonzero_matrix.detach()
            kv = self.kv_linear(x_enhance)
            mask_nonzero_matrix = torch.clone(kv)
            (batch, row) = mask_nonzero
            mask_nonzero_matrix[batch, row, :] = torch.zeros_like(mask_nonzero_matrix[0, 0, :])
            kv = kv - mask_nonzero_matrix.detach()
            k, v = torch.chunk(kv, 2, dim=-1)
        else:
            qkv = self.qkv_linear(x)
            mask_nonzero_matrix = torch.clone(qkv)
            (batch, row) = mask_nonzero
            mask_nonzero_matrix[batch, row, :] = torch.zeros_like(mask_nonzero_matrix[0, 0, :])
            qkv = qkv - mask_nonzero_matrix.detach()
...
class TransformerLayer(nn.Module):
    def __init__(self, h_in, n_head, h_hid, dropout, after_norm, dropout_attn, attn_scale,root_enhance):
        """
        ...
        self.self_attn =  MultiHeadAttn_Root_Enhence(h_in, n_head, dropout_attn, attn_scale, root_enhance)
        # self.self_posi_attn = BatchMultiHeadGraphAttention(n_head, h_in, h_in, attn_dropout= dropout_attn)
...
class TransformerEncoder(nn.Module):
    def __init__(self, num_layers, h_in, n_head, h_hid, dropout, after_norm=True, dropout_attn=None, attn_scale=False, root_enhance =True):
        super().__init__()
        ...
        self.layers = nn.ModuleList([TransformerLayer(h_in, n_head, h_hid, dropout, after_norm, dropout_attn, attn_scale, root_enhance)
...
class Decoder(nn.Module):
    def __init__(self, num_layers, h_in, n_head, h_hid, dropout, after_norm=True, dropout_attn=None, attn_scale=False, root_enhance=False):
